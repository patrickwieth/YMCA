...
if command -v curl >/dev/null 2>&1; then
    curl -s -L -O https://github.com/AppImage/AppImageKit/releases/download/continuous/appimagetool-x86_64.AppImage || { echo "Download failed with curl, falling back to 1.9.0"; curl -s -L -O https://github.com/AppImage/appimagetool/releases/download/1.9.0/appimagetool-x86_64.AppImage || { echo "Fallback download failed"; exit 1; }; }
else
    wget -cq https://github.com/AppImage/AppImageKit/releases/download/continuous/appimagetool-x86_64.AppImage || { echo "Download failed with wget, falling back to 1.9.0"; wget -cq https://github.com/AppImage/appimagetool/releases/download/1.9.0/appimagetool-x86_64.AppImage || { echo "Fallback download failed"; exit 1; }; }
fi
