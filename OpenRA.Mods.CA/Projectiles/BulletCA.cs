...
namespace OpenRA.Mods.CA.Projectiles
{
	...
	public class BulletCA : IProjectile, ISync
	{
		...
		bool ShouldExplode(World world)
		{
			...
			if (!string.IsNullOrEmpty(info.PointDefenseType) && world.ActorsWithTrait<IPointDefense>().Any(x => x.Trait.Destroy(pos, args.SourceActor.Owner, new BitSet<string>(info.PointDefenseType))))
				return true;
