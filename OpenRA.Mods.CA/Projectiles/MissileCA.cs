...
namespace OpenRA.Mods.CA.Projectiles
{
	public class MissileCAInfo : IProjectileInfo
	{
		...
		public readonly WDist ContrailStartWidth = new WDist(64);
	...
	public class MissileCA : IProjectile, ISync
	{
		...
		public void Tick(World world)
		{
			...
			if (args.GuidedTarget.IsValidFor(args.SourceActor) && lockOn)
				newTarPos = (args.Weapon.TargetActorCenter ? args.GuidedTarget.CenterPosition : args.GuidedTarget.Positions.First())
			...
			if (!shouldExplode && !string.IsNullOrEmpty(info.PointDefenseType))
				shouldExplode |= world.ActorsWithTrait<IPointDefense>().Any(x => x.Trait.Destroy(pos, args.SourceActor.Owner, new BitSet<string>(info.PointDefenseType)));
