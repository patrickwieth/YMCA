...
namespace OpenRA.Mods.CA.Widgets.Logic
{
	public static class LobbyUtils
	{
			...
			Dictionary<string, LobbyFaction> factions)
		{
			...
			SetupGameWidget(dropdown, c, factions);
		}

		public static void SetupGameWidget(Widget parent, Session.Client c, Dictionary<string, LobbyFaction> factions)
		{
			var gameName = parent.Get<LabelWidget>("GAMENAME");
			gameName.GetText = () => factions[c.Faction].Game;

			/*
			var font = Game.Renderer.Fonts[gameName.Font];
			var truncated = new CachedTransform<string, string>(clientFaction =>
				WidgetUtils.TruncateText(factions[clientFaction].Name, gameName.Bounds.Width, font));
			gameName.GetText = () => truncated.Update(factions[c.Faction].Game);
			*/
