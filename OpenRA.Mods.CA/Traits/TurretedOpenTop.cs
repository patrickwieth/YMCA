...
{
	public class TurretedOpenTopInfo : PausableConditionalTraitInfo, Requires<BodyOrientationInfo>, IActorPreviewInitInfo, IEditorActorOptions
	{
		...
		public override object Create(ActorInitializer init) { return new TurretedOpenTop(init, this); }
	}

	public class TurretedOpenTop : PausableConditionalTrait<TurretedOpenTopInfo>, ITick, IDeathActorInitModifier, IActorPreviewInitModifier
	{
		AttackTurretedOpenTopped attack;
		IFacing facing;
		...
		public TurretedOpenTop(ActorInitializer init, TurretedOpenTopInfo info)
			: base(info)
		{
		...
		{
			base.Created(self);
			attack = self.TraitsImplementing<AttackTurretedOpenTopped>().SingleOrDefault(at => ((AttackTurretedOpenToppedInfo)at.Info).Turrets.Contains(Info.Turret));
		...
		protected virtual void Tick(Actor self)
		{
			...
			// NOTE: FaceTarget is called in AttackTurretedOpenTopped.CanAttack if the turret has a target.
			if (attack != null)
			{
