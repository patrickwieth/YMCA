...
namespace OpenRA.Mods.CA.Traits
{
	...
	class DriverPoisonedByTintedCellsInfo : ConditionalTraitInfo, Requires<HealthInfo>, IRulesetLoaded
	{
		...
		public override object Create(ActorInitializer init) { return new DriverPoisonedByTintedCells(init.Self, this); }
		...
		public override void RulesetLoaded(Ruleset rules, ActorInfo info)
		{
			...
			if (DamageLevel == 0)
				throw new YamlException("DamageLevel of DriverPoisonedByTintedCells of actor \"" + info.Name + "\" cannot be 0.");

			var layers = rules.Actors["world"].TraitInfos<TintedCellsLayerInfo>()
				.Where(l => l.Name == LayerName);

			if (!layers.Any())
				throw new InvalidOperationException("There is no TintedCellsLayer named \"" + LayerName + "\" to match DriverPoisonedByTintedCells of actor \"" + info.Name + "\"");

			if (layers.Count() > 1)
				throw new InvalidOperationException("There are multiple TintedCellsLayers named \""
					+ LayerName + "\" to match DriverPoisonedByTintedCells of actor \"" + info.Name + "\"");
		}
	}

	class DriverPoisonedByTintedCells : ConditionalTrait<DriverPoisonedByTintedCellsInfo>, ITick, ISync
	{
		...
		public DriverPoisonedByTintedCells(Actor self, DriverPoisonedByTintedCellsInfo info)
			: base(info)
		{
